...
public class UnCliente implements Runnable {
    ...
    private void cerrarConexion() {
        try {
            if (autenticado) {
                List<String> juegosParaBorrar = new ArrayList<>();
                for (Map.Entry<String, gato> entry : ServidorMulti.juegosActivos.entrySet()) {
                    ...
                    if (juego.getJugador1().equals(idCliente) || juego.getJugador2().equals(idCliente)) {
                        ...
                        juegosParaBorrar.add(entry.getKey());
                    }
                }

                for (String key : juegosParaBorrar) {
                    ServidorMulti.juegosActivos.remove(key);
                }
            }
    ...
    public void run() {
        try {
                ...
                if (comando.equals("REGISTRAR") || comando.equals("ENTRAR")) {
                    ...
                    if (comando.equals("REGISTRAR")) {
                        ...
                        if (ServidorMulti.verificarCredenciales(nombre, password)) {
                            ...
                            if (ServidorMulti.clientes.containsKey(nombre)) {
                                salida.writeUTF("Error: El usuario '" + nombre + "' ya tiene una sesión activa. Cierra la otra sesión primero.");
                                continue;
                            }
            ...
            System.out.println("Se ha perdido la conexión con " + idCliente);
            cerrarConexion();
        }
    }
}